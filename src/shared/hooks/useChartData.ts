...
import {
  catmullRomToBezier,
  normalizeSeriesToPoints,
} from '@/widgets/Charts/model/math';
...
export const useChartData = (
  ...
  const { points: primarySeriesPoints, path: primarySeriesPath } =
    useMemo(() => {
      const normalizedPrimaryPoints = normalizeSeriesToPoints(
        primarySeries,
        chartWidth,
        chartHeight,
      );
  ...
  const { points: secondarySeriesPoints, path: secondarySeriesPath } =
    useMemo(() => {
